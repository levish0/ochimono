...
export class SettingsPanelFrame extends Container {
	...
	static searchY(offset: number) { return this.top + Math.max(0, this.header - offset); }
	static contentY(offset: number) { return this.searchY(offset) + this.searchHeight; }
	static viewportHeight(height: number) { return height - this.top - this.searchHeight; }
	layout(width: number, height: number, offset: number, themeProgress: number) {
		const theme = themeAt(themeProgress), y = SettingsPanelFrame.searchY(offset);
		this.surface.clear().rect(0, 40, width, height - 40).fill(theme.surface)
			.rect(0, 40, 170, height - 40).fill(theme.sidebar);
		this.heading.y = 60 - Math.min(100, offset);
		this.heading.visible = offset < 60;
		this.heading.style.fill = theme.text;
		this.search.clear().roundRect(182, y + 6, width - 204, 36, 5).fill(theme.sidebar);
