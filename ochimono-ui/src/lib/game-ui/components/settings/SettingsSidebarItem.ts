...
export class SettingsSidebarItem extends Container {
	...
	constructor(
		...
		this.on('pointerenter', () => {
			this.hovered = true;
			this.motion.to(this.visual, { hover: 1 }, 500);
			hoverSound();
		});
		this.on('pointerleave', () => {
			this.hovered = false;
			this.motion.to(this.visual, { hover: 0 }, 500);
		});
	}
	update(selected: boolean, dark: number) {
		if (this.selected !== selected) {
			...
			const elasticHalf = (t: number) =>
				t === 0 || t === 1 ? t : 1 + 2 ** (-10 * t) * Math.sin(((t - 0.15) * 2 * Math.PI) / 0.6);
			this.motion.to(
				this.visual,
				{ height: selected ? 18 : 4 },
				500,
				selected ? elasticHalf : ease.outQuint
			);
		}
		const theme = themeAt(dark);
		this.background
			.clear()
			.roundRect(5, 5, 160, 36, 5)
			.fill({ color: theme.text, alpha: this.visual.hover * 0.1 });
		this.content.tint = this.selected || this.hovered ? theme.text : theme.muted;
		this.indicator
			.clear()
			.roundRect(14, 23 - this.visual.height / 2, 4, this.visual.height, 1.5)
