...
export class ImageBackground extends Container {
	...
	update(width: number, height: number, pointer: { x: number; y: number }, dt: number, shift: number, reduced: boolean, active: boolean) {
		...
		const target = (p: number, size: number) => Math.sign(p) * (1 - 0.999 ** Math.abs(p * size)) * size / 2 * amount;
		...
		const coverScale = Math.max(width / this.image.texture.width, height / this.image.texture.height);
		this.image.scale.set(coverScale * screenScale * this.parallaxScale);
		this.image.position.set(width / 2 + this.offset.x + this.transitionX + shift, height / 2 + this.offset.y);
	}
}
