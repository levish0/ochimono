...
use config::auth::OAuthProviderCredentials;
...
pub fn generate_auth_url<P: OAuthProviderConfig>(
    ...
    let redirect_url = RedirectUrl::new(credentials.redirect_uri.to_string())
        .map_err(|_| Errors::OauthInvalidRedirectUrl)?;

    let client = BasicClient::new(ClientId::new(credentials.client_id.to_string()))
