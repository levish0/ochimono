...
pub fn log_error(error: &Errors) {
    match error {
        ...
        | Errors::AuthAlreadyAuthenticated
        | Errors::AuthProofStale
        | Errors::SignupRestricted
...
pub fn map_response(error: &Errors) -> Option<(StatusCode, &'static str, Option<String>)> {
    match error {
        ...
        Errors::AuthAlreadyAuthenticated => {
            Some((StatusCode::CONFLICT, AUTH_ALREADY_AUTHENTICATED, None))
        }
        Errors::AuthProofStale => Some((StatusCode::UNAUTHORIZED, AUTH_PROOF_STALE, None)),
        Errors::SignupRestricted => Some((StatusCode::FORBIDDEN, AUTH_SIGNUP_RESTRICTED, None)),
        Errors::UserInvalidPassword => {
